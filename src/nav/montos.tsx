...
const Montos: React.FC = () => {
  ...
  const fetchUserData = async (userName: string) => {
    if (userName.trim() !== "") {
      try {
        const response = await fetch(`https://apireact-evp2.onrender.com/api/users/search?name=${userName}`);
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://apireact-evp2.onrender.com/api/montos/register', {
        method: 'POST',
        headers: {
